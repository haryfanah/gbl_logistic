...
// if everything is ok, try to upload file
  ...
  if (move_uploaded_file($_FILES["fileUploaded"]["tmp_name"], $target_file)) {
    // ecsho "The file ". htmlspecialchars( basename( $_FILES["fileUploaded"]["name"])). " has been uploaded.";
  	$upload = $db->prepare('UPDATE tools set image=? where id_tool= ?');
  	$upload->execute(array($_FILES['fileUploaded']['name'],$idTool));
    if($idSup)
    {
      header('location:../main.php?page=pages/voir_equipe&idSup='.$idSup);
    }else{
      header('location:../main.php?page=pages/categorie&idTool='.$idTool.'&id='.$id);
    }
  } else {
